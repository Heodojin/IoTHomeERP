function [signal,sample_mask] = cleanwindows(signal,fs,max_bad_channels,zthresholds,window_len,window_overlap,max_dropout_fraction,min_clean_fraction,truncate_quant,step_sizes,shape_range)
...
if ~isempty(max_bad_channels) && max_bad_channels > 0 && max_bad_channels < 1 %#ok<*NODEF>
    max_bad_channels = round(size(signal,1)*max_bad_channels); end
...
%     warning('Falling back to a basic substitute and dropping signal meta-data.');
    signal = signal(:,sample_mask);
%     signal.pnts = size(signal,2);
%     signal.xmax = signal.xmin + (signal.pnts-1)/fs;    
